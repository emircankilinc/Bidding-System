...
your application to listen on `localhost:8080`. After running this shell script there should be a result file which contains result of application and checking everything seems fine or not.
It will issue a number of bid
...
Useful links for information below after the app is running.

- Injection approaches in this application, interface injection is used in controller and service to make the class more flexible and easier to test.
- The controller class is returning ResponseEntity with a status code and bid content.
- Swagger documentation of application : http://localhost:8080/v2/api-docs
- Health check of application : http://localhost:8080/actuator/health
  - Now only /health endpoint is exposed for application. It can be changeable or add new actuator endpoints on application.properties file.
- If the number of bidders is to be increased, desired endpoints can be added to the application.properties file.